...
def helper_sgtwodidv(w):
	'''Perform Savitzky-Golay smoothing and get 1st derivative'''
	w['XX'] = signal.savgol_filter(
			w['XX'], int(w['sgtwodidv_samples']), int(w['sgtwodidv_order']), deriv=1, delta=w['ystep'])# / 0.0129064037)
	w['cbar_quantity'] = 'd^{2}I/dV^{2}'
	w['cbar_unit'] = '$\mu$Siemens'
	w['cbar_unit'] = r'$\mathrm{e}^2/\mathrm{h}$'
	data = w['XX']
...
def helper_log(w):
	...
	w['cbar_quantity'] = 'log$_{10}$|' + w['cbar_quantity'] + '|'
